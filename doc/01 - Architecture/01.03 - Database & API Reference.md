...
type: reference
tags: [reference, database, api, pgvector, minio, type/reference, status/active]
status: active
updated: 2026-05-17
sources: [DATABASE_API_REFERENCE.md, DATABASE_API_ARCHITECTURE.md]
related: ["[[01.01 - System Architecture]]", "[[02.02 - Agent - SQL Specialist]]", "[[03.04 - Document Upload & RAG]]"]
---

# 01.03 - Database & API Reference

> **Unified DB schema + API reference.** Merges the broad cross-component schema view (Chat-/ChatV1 app tables + Agent analytics tables) with the deeper technical detail on connection pools, pgvector configuration, and migration history.
...
| เอกสาร | ตำแหน่ง |
|--------|---------|
| Agent Architecture | [[01.01 - System Architecture]] |
| Agent Architecture | [[01.01 - System Architecture]] |
| Agent Existing DB Architecture | [[01.03 - Database & API Reference]] |
...
│  │                        │  │  dim_source                     │ │
│  │  ─── Domain Data ───   │  │                                 │ │
│  │  diabetes              │  │  fact_accident_event            │ │
│  │  bipola                │  │  fact_accident_person (empty)   │ │
│  │  (CSV-imported tables) │  │                                 │ │
│  │                        │  │  mart_accident_summary          │ │
│  │  accident: DROPPED     │  │  mart_accident_hotspot          │ │
│  └────────────────────────┘  │  mart_province_year             │ │
│    (migration 016)           │  mart_province_road             │ │
│                              │                                 │ │
...
> **⚠️ `accident` table was DROPPED in Migration 016** (`016_drop_accident_mockup.sql`).  
> It was a flat denormalized mockup (100 rows, Thai BE years 2567-2568). All real accident data lives in `fact_accident_event` (star schema, 2020–2026 CE). Use `fact_accident_event` + `dim_geography` for all accident queries.
...
| Column | Type | Nullable | Default | คำอธิบาย |
|--------|------|----------|---------|---------|
| `geography_id` | BIGSERIAL (PK) | NOT NULL | auto | Primary key |
| `country_code` | VARCHAR(10) | YES | `'TH'` | รหัสประเทศ |
| `province_code` | VARCHAR(20) | YES | — | รหัสจังหวัด |
| `province_name` | VARCHAR(255) | YES | — | จังหวัด |
| `district_code` | VARCHAR(20) | YES | — | รหัสอำเภอ |
| `district_name` | VARCHAR(255) | YES | — | อำเภอ |
| `subdistrict_code` | VARCHAR(20) | YES | — | รหัสตำบล |
| `subdistrict_name` | VARCHAR(255) | YES | — | ตำบล |
| `latitude` | NUMERIC(10,6) | YES | — | ละติจูด |
| `longitude` | NUMERIC(10,6) | YES | — | ลองจิจูด |

**UNIQUE**: `uq_dim_geography_province_district_sub` on (province_name, district_name, subdistrict_name) WHERE province_name IS NOT NULL  
**Indexes**: `idx_geo_province` (province_name), `idx_geo_district` (district_name)  
**Note**: NULL values in district_name/subdistrict_name normalized to `''` (Migration 017) so composite unique index works correctly
...
#### `document_registry` — ทะเบียนเอกสาร (Migration 002 + 012 + 013 + 016b)
| Column | Type | Default | คำอธิบาย |
|--------|------|---------|---------|
| `document_id` | BIGSERIAL (PK) | auto | Primary key |
| `title` | VARCHAR(500) | — | ชื่อเอกสาร |
| `topic` | VARCHAR(100) | — | โดเมน (accident, mental_health) |
| `document_type` | VARCHAR(100) | — | ประเภท (PDF/DOCX) |
| `source_id` | BIGINT (FK) | — | FK → dim_source |
| `effective_date` | DATE | — | วันที่มีผล |
| `version_no` | VARCHAR(50) | — | เวอร์ชัน |
| `status` | VARCHAR(50) | `'active'` | สถานะ (active/archived) |
| `uploaded_at` | TIMESTAMP | `NOW()` | วันที่อัปโหลด |
| `file_path` | VARCHAR(1000) | — | path ใน MinIO |
| `minio_path` | VARCHAR(1000) | `''` | full MinIO object path |
| `total_pages` | INTEGER | — | จำนวนหน้า |
| `original_url` | VARCHAR(1000) | — | URL ต้นฉบับ |
| `open_url` | VARCHAR(1000) | — | /api/documents/open/{id} |
| `file_size` | BIGINT | `0` | ขนาดไฟล์ (bytes) |
| `content_type` | VARCHAR(100) | `''` | MIME type |
| `upload_method` | VARCHAR(20) | `'minio'` | minio \| url \| direct |
| `uploaded_by` | VARCHAR(100) | `''` | ผู้อัปโหลด |
| `ingestion_status` | VARCHAR(20) | `'pending'` | pending \| processing \| completed \| failed |
| `chunk_count` | INTEGER | `0` | จำนวน chunk ที่ embed แล้ว |
| `error_message` | TEXT | `''` | ข้อความ error ถ้า ingestion ล้มเหลว |
| `source_type` | VARCHAR(20) | `'internal'` | internal \| external |
| `external_url` | VARCHAR(2000) | `''` | URL ถ้าดึงจาก external |
| `apa_type` | VARCHAR(50) | `'report'` | ประเภท APA: article, book, report |
| `apa_authors` | TEXT | `''` | ผู้แต่ง (APA format) |
| `apa_year` | VARCHAR(50) | `''` | ปีพิมพ์ (supports ranges like "2020-2023") |
| `apa_publisher` | VARCHAR(500) | `''` | สำนักพิมพ์/วารสาร |
| `apa_doi` | VARCHAR(200) | `''` | DOI |
| `apa_url` | VARCHAR(1000) | `''` | URL อ้างอิง APA |
| `apa_accessed` | DATE | — | วันที่เข้าถึง |
| `apa_edition` | VARCHAR(100) | `''` | ฉบับที่ |
| `apa_volume` | VARCHAR(50) | `''` | เล่มที่ |
| `apa_pages` | VARCHAR(50) | `''` | หน้า |
| `apa_approval_status` | VARCHAR(20) | `'pending'` | pending \| draft \| approved \| rejected |

**Indexes**: `idx_doc_topic` (topic), `idx_doc_registry_status` (ingestion_status), `idx_doc_registry_uploaded` (uploaded_at DESC), `idx_doc_registry_source_type` (source_type)  
...
#### `evidence_registry` — ทะเบียนหลักฐาน (Migration 010 + 015 + 017)

| Column | Type | Nullable | Default | คำอธิบาย |
|--------|------|----------|---------|---------|
| `evidence_id` | VARCHAR(20) | NOT NULL | — | Primary key (EV-001) |
| `session_id` | VARCHAR(255) | YES | — | Chat session ที่สร้าง |
| `evidence_type` | VARCHAR(20) | NOT NULL | — | `document` \| `database` \| `api` \| `thaijo_article` |
...
| `thaijo_pdf_url` | TEXT | YES | — | URL ของ PDF บน ThaiJO (Migration 015) |
| `thaijo_reference` | TEXT | YES | — | APA reference string จาก ThaiJO (Migration 015) |
| `thaijo_search_term` | TEXT | YES | — | คำค้นหาที่ใช้ค้น ThaiJO (Migration 015) |

**Check constraint**: `evidence_type IN ('document','database','api','thaijo_article')` — fixed in Migration 017 (ลบ `notebooklm_pdf` ออก)  
**Check constraint**: `trust_level IN ('high','medium','low')`  
**Indexes**: `idx_ev_session`, `idx_ev_source`, `idx_ev_topic`, `idx_ev_query_sig`, `idx_evidence_thaijo` (partial index on evidence_type='thaijo_article')

> **⚠️ Migration 021 NOT YET APPLIED** — columns `apa_type`, `apa_authors`, `apa_year`, `apa_publisher`, `thaijo_summary` are defined in `021_evidence_registry_apa_thaijo.sql` but have NOT been run against the live DB. Run `021_evidence_registry_apa_thaijo.sql` before using those fields.
...
#### `thaijo_search_cache` — ThaiJO Search Cache (Migration 018)

| Column | Type | Nullable | Default | คำอธิบาย |
|--------|------|----------|---------|---------|
| `cache_key` | VARCHAR(64) (PK) | NOT NULL | — | sha256(normalized_term\|size=N) |
| `search_term` | TEXT | NOT NULL | — | คำค้นหาต้นฉบับ |
| `normalized_term` | TEXT | NOT NULL | — | คำค้นหาหลัง normalize |
| `results_json` | JSONB | NOT NULL | — | raw API results array (source of truth for URL verification) |
| `result_count` | INTEGER | YES | `0` | จำนวน results |
| `api_called_at` | TIMESTAMP | NOT NULL | `NOW()` | เวลาที่เรียก API |
| `expires_at` | TIMESTAMP | NOT NULL | — | เวลาหมดอายุ (NOW() + THAIJO_CACHE_TTL_DAYS) |
| `hit_count` | INTEGER | YES | `0` | จำนวนครั้งที่ cache ถูกใช้ |
| `last_hit_at` | TIMESTAMP | YES | — | ครั้งล่าสุดที่ใช้ cache |

**Indexes**: `idx_thaijo_cache_term` (normalized_term), `idx_thaijo_cache_expires` (expires_at), `idx_thaijo_cache_pdf_urls` GIN (results_json)  
**Purpose**: เก็บผล ThaiJO API ทันทีที่ได้รับ ก่อนส่งให้ LLM — ใช้ verify PDF URL ใน `evidence_registry.open_url`
...
| **015** | `015_thaijo_evidence.sql` | ✅ ADD `thaijo_pdf_url`, `thaijo_reference`, `thaijo_search_term` to `evidence_registry` |
| **016a** | `016_drop_accident_mockup.sql` | ✅ DROP TABLE `accident` (flat mockup — all real data in fact_accident_event) |
| **016b** | `016_widen_apa_year.sql` | ✅ ALTER `document_registry.apa_year` VARCHAR(10) → VARCHAR(50) (supports ranges like "2020-2023") |
| **017a** | `017_fix_evidence_type_constraint.sql` | ✅ Expand `evidence_type` CHECK to include `thaijo_article` |
| **017b** | `017_geography_subdistrict_constraint.sql` | ✅ Replace province-only UNIQUE with composite `(province_name, district_name, subdistrict_name)` |
| **018** | `018_thaijo_search_cache.sql` | ✅ CREATE TABLE `thaijo_search_cache` (applied manually, not in migration_history) |
| **019** | `019_cleanup_thaijo_evidence.sql` | ⚙️ Data cleanup only — clears unverified `open_url` for ThaiJO entries. Run after cache is populated. |
| **020** | `020_fix_evidence_download_urls.sql` | ⚙️ Data cleanup only — normalizes `/download/NNNNN` and `/view/N/M` URL patterns in evidence_registry |
| **021** | `021_evidence_registry_apa_thaijo.sql` | ⏳ **NOT YET APPLIED** — ADD `apa_type`, `apa_authors`, `apa_year`, `apa_publisher`, `thaijo_summary` to `evidence_registry` |

**Legend**: ✅ Applied to live DB · ⚙️ Data-only (no schema change) · ⏳ Pending

**วิธีรัน Migration (Manual)**:
```bash
psql -U postgres -d chat-aio -f database/001_shared_core.sql
psql -U postgres -d chat-aio -f database/002_document_rag.sql
# ... ไปจนถึง 021
psql -U postgres -d chat-aio -f database/021_evidence_registry_apa_thaijo.sql
...
### 7.1D Obsidian APIs (✅ Implemented)

| Method | Path | คำอธิบาย |
|--------|------|---------|
| `POST` | `/api/obsidian/search` | ค้นหาข้อมูลใน Obsidian Vault |
| `GET` | `/api/obsidian/notes` | ดึงรายการ notes ทั้งหมด |
| `POST` | `/api/obsidian/ask` | ถามตอบโดยใช้ Obsidian RAG |
| `POST` | `/api/obsidian/ask/stream` | ถามตอบโดยใช้ Obsidian RAG (Stream) |
| `GET` | `/api/obsidian/status` | ตรวจสถานะระบบ Obsidian |

### 7.1E Accident Policy/Chat APIs (✅ Implemented)

| Method | Path | คำอธิบาย |
|--------|------|---------|
| `POST` | `/api/accident-chat/ask` | รัน Accident Chat Pipeline |
| `POST` | `/api/accident-chat/quick` | ดึงข้อมูลดิบจากเครื่องมือ |
| `GET` | `/api/accident-policy/zone10/data` | ดึงข้อมูลรายงานสถิติ |
| `POST` | `/api/accident-policy/zone10` | รัน Accident Policy Pipeline |
...
## Appendix — Technical Deep-Dive (from DATABASE_API_ARCHITECTURE.md)

> **Scope.** This appendix preserves the deeper implementation-level material from the original `DATABASE_API_ARCHITECTURE.md` that is not already covered above: connection-pool internals, the pgvector module's exact behaviour, the per-migration changelog with the `evidence_type` CHECK-constraint warning, and the CSV ingestion script details. Operational best practices and troubleshooting tips are also included here for reference.

### A1. Connection Pool Architecture (`src/db/pool.py`)

The Agent backend maintains **two separate connection pools** because FastAPI endpoints run on asyncio while CrewAI agent tools run synchronously inside thread workers. Sharing a single pool across both worlds is not possible without bridging code, so the cleanest design is two singletons.

#### A1.1 Async Pool (for FastAPI endpoints)
- **Library**: `asyncpg`
- **Pool Size**: 2-10 connections
- **Usage**: API endpoints, async operations
- **Lifecycle**: Created on app startup, closed on shutdown (managed via FastAPI lifespan)
...
#### A1.2 Sync Pool (for CrewAI tools)
...
#### A1.3 `query_db()` Helper

All agent tools go through a single thin wrapper rather than checking out connections themselves. This guarantees pool hygiene and a consistent row-as-dict return shape.
...
### A2. pgvector Module Specifics (Migration 011)

In addition to the schema documented in §4.5/§5, the architecture document records the following operational specifics:

- **`document_chunks` legacy table** — the original RAG design (Migration 002) included a `document_chunks` table (fields: `chunk_text`, `keywords`, `embedding_ref`, `page_ref`). It was never populated by the pgvector pipeline (current row count: 0) and was finally dropped in Migration 014. All chunk text + embeddings live in `document_embeddings`.
- **Upsert semantics** — `ON CONFLICT (id) DO UPDATE` makes re-ingestion idempotent, where `id` is the MD5 hash of `source::chunk_index`.
- **Search operator** — Cosine distance via the `<=>` operator: `ORDER BY embedding <=> query_vec`.
- **Vector index status** — pgvector HNSW/IVFFlat indexes are capped at **2000 dimensions**. Because the embeddings are **3072-dim** (Gemini `gemini-embedding-001`), there is **no ANN index**; queries run an exact sequential cosine scan, which is fine for `<50K` chunks.
- **Tooling** — Same connection as all other tables, so embeddings are inspectable directly in DBeaver/pgAdmin with no separate vector DB tool.
- **Reference reading** at the time the architecture document was written: **6 chunks across 4 documents** ingested from MinIO.

> **⚠️ Historical note on dimension/index.** Earlier drafts of the architecture document described the embedding column as `vector(768)` with an HNSW index. That predates the migration to Gemini `gemini-embedding-001`. The current, correct configuration is `vector(3072)` with no ANN index — see §4.5/§5 above for the authoritative spec.

### A3. Migration History (Sequential, with Notes)

This is the per-migration changelog from the architecture document. It covers the same 001–015 sequence summarized in §4.6 but adds the `evidence_type` CHECK-constraint warning and emphasizes the role of materialized views from Migration 007.
...
> **✅ `evidence_type` CHECK constraint:** Fixed in Migration 017. Current allowed values: `('document', 'database', 'api', 'thaijo_article')`. Note: `notebooklm_pdf` is NOT in the constraint — do not use it.

#### A3.1 Materialized Views (Migration 007)

Migration 007 added two **materialized views** alongside the schema enhancements, intended for fast province-level queries without joining `dim_geography` at query time:

1. **`v_province_year_summary`** — Denormalized view of `mart_province_year` + `dim_geography`. Includes `latitude`, `longitude` for mapping.
2. **`v_province_road_year`** — Denormalized view of `mart_province_road`. Direct access to province road statistics.

### A4. CSV Ingestion Flow — `database/import_subdistrict_csv.py`

The CSV import script is the canonical path for loading 2020–2026 accident data and rebuilding the mart layer. (This is a separate path from the document/MinIO ingestion in §9.3.)
...
- **Duplicate handling**: Upsert with `ON CONFLICT` (paired with the UNIQUE constraints from Migration 008).
- **Data validation**: Skip invalid rows, log errors.
- **Incremental**: Can be re-run without duplicating data.

### A5. Query Flow Patterns

#### A5.1 Agent Tool Query Flow
...
#### A5.3 Aggregation Query Example — Hour-of-day Distribution
...
### A6. Data Storage Strategy

- **Fact tables (raw data)** — Row-oriented (PostgreSQL default); retain all historical data (2020–2026+); append-only (no updates to historical facts); no partitioning today, but partition by year when fact tables exceed ~1M rows.
- **Mart tables (aggregated)** — Row-oriented with indexes; rebuilt on CSV import; materialized aggregates (not views) to keep agent-tool queries fast.
- **Vector embeddings (`document_embeddings`)** — PostgreSQL `vector(3072)` column (pgvector); no ANN index because 3072 > the pgvector 2000-dim cap → exact cosine sequential scan; `<=>` cosine operator; `ON CONFLICT DO UPDATE` for idempotent re-ingestion; viewable in DBeaver/pgAdmin alongside every other table.
- **Index strategy** — Time-based: `(year_no, month_no)` for time-series. Geographic: `province_name`, `geography_id`. Scoring: `hotspot_score DESC` for top-N. Foreign keys are indexed automatically.

### A7. Database Configuration (`src/config.py`)
...
4. **Cache mart queries** with Redis for frequently accessed aggregates.
...
Solution: Check tool returns valid JSON, verify mart tables have data,
         run verify_province_data.py
```

**4. CSV import fails**
```
Issue: import_subdistrict_csv.py crashes
Solution: Check CSV encoding (UTF-8), verify column count matches expected schema,
         check for NULL geography_id
```

### A10. Future Enhancements (Roadmap Notes)

1. **Read replicas** — separate read/write pools for scalability.
2. **Partitioning** — partition `fact_aident_event` by year.
3. **Materialized views** — auto-refresh marts on a schedule.
4. **Query caching** — Redis cache for common aggregations.
5. **Time-series DB** — consider the TimescaleDB extension.
6. **Full-text search** — add `tsvector` columns for text search in documents.
7. **pgvector IVFFlat** — s switch from sequential cosine to IVFFlat once embeddings drop ≤2000-dim, or for very large embedding sets (>1M rows).

### A11. Related Source Pointers

- **API routes** — `src/routers/chat.py`, `src/routers/test_ui.py`
- **Agent tools** — `src/tools/chart_builder.py`, `src/tools/accident.py` (see [[02.01 - Agent Workflows]] and [[02.02 - Agent - SQL Specialist]])
- **Database migrations** — `database/001_*.sql` through `database/015_thaijo_evidence.sql`
- **Vector store** — `src/rag/vector_store.py` (pgvector implementation)
- **RAG pipeline** — `src/rag/document_rag.py` (MinIO → pgvector ingestion; see [[03.04 - Document Upload & RAG]])
- **Data import** — `database/import_subdistrict_csv.py`
- **Schema analysis** — `database/analyze_schema.py`, `database/verify_province_data.py`
- **Full DB reference** — this document ([[01.03 - Database & API Reference]])
