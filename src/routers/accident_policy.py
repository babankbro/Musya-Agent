...
from src.agents.accident_policy_orchestrator import (
    run_zone10_analysis,
...
async def get_zone10_data(
    provinces: str = Query(
        default="",
        description="Comma-separated Zone 10 province names (Thai). Empty = all 5.",
    )
):
    """Return raw SQL query results for all 7 policy questions — no LLM, fast (<5s).

    Use this endpoint for UI data preview or testing the SQL tools directly.
    """
    province_list = (
    ...
    if invalid:
        raise HTTPException(
            status_code=400,
            detail={
                "error": "invalid_provinces",
                "message": f"จังหวัดไม่ถูกต้อง: {invalid}",
                "valid_provinces": list(ZONE10_PROVINCES),
            },
        )
    ...
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail={"error": "data_query_failed", "message": str(exc)},
        )
...
async def create_zone10_policy_brief(request: AccidentPolicyRequest):
    ...
    Produces a Thai government-style policy report for สสส./สสจ./ศปถ.

    Expected runtime: 2–5 minutes (LLM pipeline).
    """
    logger.info(
        "[accident-policy] Starting: provinces=%s questions=%s year_range=%s",
